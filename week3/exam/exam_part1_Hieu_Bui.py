...
def count_spaces(string):
    ...
    *** Important: your code must use comprehensions and should not be more than
    ...
    with just spaces.
    """
    count_spaces = [ s for s in string if s == " "]
...
def build_heights_dict():
    ...
    with open("heights.txt") as file:
        for line in file:
            line=line.strip()
            tokens = line.split()
            dict[tokens[0] + " " + tokens[1]] = int(tokens[2])
    
    return dict


print('not_string("cat") expected: notcat')
print('not_string("catnot") actual:', not_string("catnot"))

print('swap([5,4,3,2,1]) expected: [1, 4, 3, 2, 5]')
print('swap([6,7,8,9,10]) actual:',swap([6,7,8,9,10]))
...
print('count_spaces("U need to pick me up, I will be there in 25") actual:', count_spaces("U need to pick me up, I will be there in 25"))
